...
void GraphicUI::displayField(const Field &field, bool isEnemiesField, std::string name, std::string notification) {
...
void GraphicUI::displayNotification(std::string notification) {
  window.clear(sf::Color::Blue);
  sf::Font font;
  font.loadFromFile("../src/fonts/OpenSans-SemiBoldItalic.ttf");
...
void GraphicUI::waitForNextTurn() {
  double start = clock();
  while (window.isOpen()) {
    sf::Event event;
    while (window.pollEvent(event)) {
...
Position GraphicUI::getPosition() {
